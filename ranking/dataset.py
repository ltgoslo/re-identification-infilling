...
class NegativeReducedDataset(Dataset):
    ...
    def __init__(self, data, labels, num_negative_labels=50, unique_labels=None):
        self.data = []
        self.labels = []
        data_len = len(data)
        if unique_labels is None:
            ...
            unique_len = len(unique_labels)
            for i, d in enumerate(data):
                correct = labels[i]
                pos = unique_labels.index(correct)
                label_ids = np.random.choice(unique_len, min(num_negative_labels, unique_len), replace=False)
                for label_id in label_ids:
                    if label_id >= pos and pos != (unique_len - 1) and label_id != (unique_len - 1):
                        self.data.append(d)
                        self.labels.append(labels[label_id+1])
                    elif label_id >= pos and (pos == (unique_len - 1) or label_id == (unique_len - 1)):
                        self.data.append(d)
                        self.labels.append(labels[0])
                    else:
                        self.data.append(d)
