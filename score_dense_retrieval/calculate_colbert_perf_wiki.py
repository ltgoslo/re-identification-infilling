...
def retrieve(text, retriever, query_tokenizer, doc_encodings, chunks, mask_index, target, mas_indices, curr_pos, ents, device="cpu", num_char=200):
    ...
    for i, chunk in enumerate(retrieved_chunks):
        for e in ents:
            search_ent = re.escape(e)
            if re.search(r"\W{}\W".format(search_ent), chunk) is not None:
                pos = i + 1
                found = True
                break
            elif re.search(r"^{}\W".format(search_ent), chunk) is not None:
                pos = i + 1
                found = True
                break
            elif re.search(r"\W{}$".format(search_ent), chunk) is not None:
                ...
                found = True
                break
            elif re.search(r"^{}$".format(search_ent), chunk) is not None:
                pos = i + 1
                found = True
                break
        if found:
...
retriever.load_state_dict(torch.load(args.colbert_model, map_location="cpu"), strict="True")
retriever.eval()
retriever.to(device)
...
for i, datapoint in enumerate(raws):
    ...
    expanded_span_list = datapoint["expanded_masked_spans"]
    ...
    for i, mask_index in enumerate(mask_indices):
        target = span_list[i]
        ents = expanded_span_list[i]
        local_text, pos, retrieved_chunks = retrieve(text, retriever, query_tokenizer, doc_encodings, chunks, mask_index, target, mask_indices, i, ents, device=device)

        curr["doc_id"] = datapoint["title"]
