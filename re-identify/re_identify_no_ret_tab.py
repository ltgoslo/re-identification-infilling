...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--input_file', type=str, default="../datasets/Anonymized_TAB_with_general_rets.jsonl",
                        help='The batch size to use during probing')
    parser.add_argument('--output_file', type=str, default="../datasets/Re-identified_TAB_no_ret.jsonl",
                        help='The batch size to use during probing')
    parser.add_argument('--results_file', type=str,
                        default='results_tab_no_ret_run_1.txt', help='The pretrained model to use')
...
def re_identify_one_span(text, reader, reader_tokenizer, num_chunks, span_list, entity_type_list, identifier_type_list, re_id_spans, order_id, device="cpu", num_char=200):
    mask_indices = []
    ...
    with torch.no_grad():
        ...
        gen_inputs = reader_tokenizer.build_inputs_for_generation(inputs, max_gen_length=128).to(device)
        x = reader.generate(**gen_inputs, eos_token_id=reader_tokenizer.eop_token_id, attention_mask=None, max_new_tokens=128)
...
# searcher = LuceneSearcher('../indexes_tab_all')
...
for i, datapoint in enumerate(raws):
    ...
    for _ in range(len(mask_indices)):
        file_to_re_id, correct, span_list, entity_type_list, entity_type, identifier_type_list, identifier_type, re_id_list, correct_tokens, total_tokens, order_id = re_identify_one_span(file_to_re_id, reader, reader_tokenizer, 2, span_list, entity_type_list, identifier_type_list, re_id_list, order_id, device)
        num_correct += correct
