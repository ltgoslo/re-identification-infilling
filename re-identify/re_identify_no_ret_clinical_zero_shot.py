...
def re_identify_one_span(text, retriever, query_tokenizer, doc_encodings, chunks, reader, reader_tokenizer, num_chunks, span_list, ner_list, re_id_spans, order_id, device="cpu", num_char=200):
    prompt = """Re-identify the fill in the blank (marked with [MASK]) in the text below, only give the value of the [MASK], do not add extra text, give explanations, or output the blank token [MASK]:
...
Answer:"""
    ...
    prompt = prompt.format(text=local_text_read)
...
raws = [json.loads(line) for line in open("../datasets/Anonymized_Clinical_notes_with_all_rets.jsonl", "r")]
...
with open("results_clinical_notes_no_ret_zero_shot_run_3.txt", "w") as f:
...
with open("../datasets/Re-identified_Clinical_notes_No_Ret_Zero_Shot.jsonl", "w") as fo:
